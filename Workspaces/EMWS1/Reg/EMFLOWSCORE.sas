...
*** Generate dummy variables for Outcome ;
drop _Y ;
label F_Outcome = 'From: Outcome' ;
length F_Outcome $ 12;
F_Outcome = put( Outcome , BEST. );
%DMNORMIP( F_Outcome )
if missing( Outcome ) then do;
   _Y = .;
end;
else do;
   if F_Outcome = '1'  then do;
      _Y = 0;
   end;
   else if F_Outcome = '0'  then do;
      _Y = 1;
   end;
   else do;
      _Y = .;
...
if (_Y = .) then do;
   ...
   R_Outcome0 = - _P1;
   select( _Y );
      when (0)  R_Outcome1 = R_Outcome1 + 1;
      when (1)  R_Outcome0 = R_Outcome0 + 1;
   end;
end;
